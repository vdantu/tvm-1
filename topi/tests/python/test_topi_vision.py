...
def verify_non_max_suppression(np_data, np_valid_count, np_indices, np_result, np_indices_result,
    ...
    indices = tvm.placeholder((batch, num_anchors), dtype="int32", name="indices")
    ...
    def check_device(device):
        ...
        with tvm.target.create(device):
            if device == 'llvm':
                out = non_max_suppression(data, valid_count, indices, -1, iou_threshold, force_suppress, top_k,
